...
def play_game(max_steps: int = 1000, verbose: bool = False) -> Tuple[bool, int]:
    """
...
def play_multiple_games(num_games: int = 100, max_steps: int = 1000) -> None:
    """
    ...
    for i in range(num_games):
        ...
        win, steps = play_game(max_steps=max_steps)
        if win:
            wins += 1
        total_steps += steps
...
def main() -> None:
    ...
    if args.games == 1:
        win, steps = play_game(max_steps=args.max_steps, verbose=args.verbose)
        print(f"Game {'won' if win else 'lost'} after {steps} steps")
    else:
        play_multiple_games(num_games=args.games, max_steps=args.max_steps)
